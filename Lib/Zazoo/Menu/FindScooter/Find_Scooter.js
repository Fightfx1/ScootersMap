    ...
    convo.ask((convo) => {
        ...
        if (payload.postback.payload === 'Bird')
        {
            if(new new require('../../../ScootersCompenys/birds/BirdUsers.js').BirdUsers().Know(payload.sender.id))
            {
                ...
                RiderOrCharger(convo, Bot); // Sending the user to say what he serach
            }
            else
            {
...
const RiderOrCharger = (convo ,Bot) => {
    convo.ask((convo) => {
        const buttons = [
            {type: 'postback', title: 'Rider', payload: 'Rider'},
            {type: 'postback', title: 'Charger', payload: 'Charger'}
        ];
            convo.sendButtonTemplate(`Are you a rider or a charger?`, buttons);
        },(payload, convo, data) => {
        if (payload.postback.payload === 'Rider')
        {
            convo.set('Type','Rider'); // Upadte that the user is Rider
            AskLocation(convo, Bot);
        }
        else if(payload.postback.payload === 'Charger')
        {
            if(convo.get('operator') == 'Bird')
            {
                new Bird(payload.sender.id).CanCharge().then(function(CanCharge){
                    ...
                    else
                    {
                        ...
                        RiderOrCharger(convo,Bot) // return the user back to the Rider Charger q;   
                    }
                })
            }
            else
