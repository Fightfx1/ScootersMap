...
function LimeRiderMap(app)
{
    app.get('/Map/LimeRider', function(req, res){
            
        function GETMap(List,Latitude,Longitude,myLocation={latitude:Latitude,longitude:Longitude})
        {
            
            let closest = {latitude:List.latitude, longitude:List.longitude}; // The Closest Scooter
            let zoomLevel = 18;

            let Scooters = []

            Scooters.push({brand:"Lime",latitude:List.latitude,longitude:List.longitude})
        ...
        if(req.query.longitude != undefined && req.query.ChatId != undefined && req.query.latitude != undefined) // Chack That theren't miss parameter.
        {
            ...
            Lime.FindScooterRider(latitude,longitude).then(function(scooters){
                res.send(GETMap(scooters,latitude,longitude));
            })
        }
    })
}

module.exports.LimeRiderMap = LimeRiderMap;
